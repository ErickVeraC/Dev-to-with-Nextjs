...
export default function Form() {
  ...
  const handleTagInput = (event) => {
    if (event.key === " " && event.target.value.trim()) {
      ...
      if (!tagsArray.includes(newTag) && tagsArray.length < 4) {
    ...
    <section className="container w-3/5 ml-10 py-10">
      <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
        <div className="bg-white shadow-md rounded-lg p-6">
          ...
          <div className="mb-4">
            <textarea
              className="w-full p-3 text-lg rounded-md focus:outline-none focus:ring focus:ring-indigo-500"
              placeholder="Write your post content here..."
              {...register("body", { required: true })}
            />
            {errors.body && <p className="text-red-500">Content is required</p>}
          </div>
        </div>
        ...
        {successMessage && (
          <div id="success-message" className="text-green-500 text-center mt-4">
